...
def get_ehealth_data(**context):
    ...
    try:
        ...
        ehealth_file_id = Variable.get(
            "EHEALTH_DATAWH_FILE_ID", default_var=None)

        if not ehealth_file_id:
            raise ValueError(
                "Missing required Airflow variable: EHEALTH_DATAWH_FILE_ID"
            )
        ...
        values = gsheets_hook.get_values(
            spreadsheet_id=ehealth_file_id, range_=sheet_name
        )
        ...
        ehealth_s3_path = Variable.get("EHEALTH_DATAWH_S3_PATH")
