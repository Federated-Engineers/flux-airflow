...
def _get_patient_hospital_visit_data(**context):
    ...
    try:
        ...
        patient_file_id = Variable.get(
            'PATIENT_HOSPITAL_VISIT_FILE_ID',
            default_var=None)
        ...
        logger.info(
            f"Downloading CSV from Google Drive, file ID: {patient_file_id}")

        # Download the file content into a memory buffer
        file_handle = io.BytesIO()
        drive_hook.download_file(
            file_id=patient_file_id, file_handle=file_handle)

        # Load buffer directly into DataFrame
        # Reset the file pointer to the beginning, read the data from the start
        file_handle.seek(0)
        ...
        logger.info(
            f"Patient hospital visit data loaded: \
            {len(patient_hospital_visit_df)} rows"
        )
